  ...
  <div class="sink-source-container">
    ...
    <el-dialog title="修改页面" :visible.sync="ModifyPageVisible" width="50%" :before-close="handleModifyClose"
      ...
      <el-form :model="modifyNodeData" status-icon ref="modifyNodeData" label-width="130px" class="ModifyNodeInfo"
        ...
        <el-form-item label="网络id" prop="network_id">
          <el-select filterable class="netid_modify_node" v-model="modifyNodeData.network_id" placeholder="请选择网络">
            　　　　<el-option v-for="item in network_id_option" :key="item.id" v-bind:value="item.id.toString()">
              {{item.name}}</el-option>
          </el-select>
        </el-form-item>
    ...
    <el-dialog title="请填入新增节点的信息：" :visible.sync="AddNodePageVisible" center :append-to-body="0" :lock-scroll="true"
      ...
      <el-form :model="addNodeData" status-icon ref="addNodeData" label-width="130px" class="AddNodeInfo"
        ...
        <el-form-item label="网络id" prop="network_id">
          <el-select filterable class="netid_add_node" v-model="addNodeData.network_id" placeholder="请选择网络">
            　　　　<el-option v-for="item in network_id_option" :key="item.id" v-bind:value="item.id.toString()">
              {{item.name}}</el-option>
          </el-select>
        </el-form-item>
  ...
  export default {
    ...
    data() {
      return {
        ...
        role_add_modify: [  // 新增或修改时role的下拉框
          {
            indexId: 1,
            name: "sink",
          },
